...
const initialState: TopicArticlesState = {
  articles: [],
  loading: false,
  error: null,
  lastVisible: null,
...
export const fetchTopicArticles = createAsyncThunk(
  'topicArticles/fetchTopicArticles',
  async (
...
const topicArticlesSlice = createSlice({
  name: 'topicArticles',
  initialState,
  reducers: {
    resetTopicArticles(state) {
      state.articles = [];
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchTopicArticles.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchTopicArticles.fulfilled, (state, action) => {
      ...
      .addCase(fetchTopicArticles.rejected, (state, action) => {
...
export const { resetTopicArticles } = topicArticlesSlice.actions;
export default topicArticlesSlice.reducer;
